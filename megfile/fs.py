...
def fs_walk(path: MegfilePathLike) -> Iterator[Tuple[str, List[str], List[str]]]:
    """
...
def _fs_scan(pathname: MegfilePathLike, missing_ok: bool = True) -> Iterator[str]:
...
def fs_scan(pathname: MegfilePathLike, missing_ok: bool = True) -> Iterator[str]:
    """
...
def fs_scan_stat(
    pathname: MegfilePathLike, missing_ok: bool = True
) -> Iterator[FileEntry]:
    """
    Iteratively traverse only files in given directory, in alphabetical order.
...
def fs_iglob(
    ...
    """
    return _create_missing_ok_generator(
        iglob(fspath(pathname), recursive=recursive),
        missing_ok,
        FileNotFoundError("No match file: %r" % pathname),
    )
