...
def _glob_with_common_wildcard():
    ...
    assert_glob(
        '/bucketForGlobTest/2/a/d/2.json', ['/bucketForGlobTest/2/a/d/2.json'])
...
def _glob_with_dironly():
    ...
    assert_glob('/bucketForGlobTest/[2-9]/', [
        '/bucketForGlobTest/2/',
    ])
...
def test_globlize():
    ...
    new_path_list = list(fnmatch.filter(path_list, glob.globlize(path_list)))
    assert "/{bucketForGlobTest/1/1,bucketForGlobTest/1/a/1,bucketForGlobTest1/b1}.json" == glob.globlize(
        path_list)
    assert path_list == new_path_list
    ...
    new_path_list = list(fnmatch.filter(path_list, glob.globlize(path_list)))
    assert "{https://bucketForGlobTest/1/a/1,s3://bucketForGlobTest/1/1,s3://bucketForGlobTest1/b1}.json" == glob.globlize(
        path_list)
    assert path_list == new_path_list
    ...
    new_path_list = list(fnmatch.filter(path_list, glob.globlize(path_list)))
    assert "s3://bucketForGlobTest/{a.nori,b.nori}/data" == glob.globlize(
        path_list)
    assert sorted(path_list) == new_path_list

    path_list = [
        'https://baidu.com/file',
    ...
    new_path_list = list(fnmatch.filter(path_list, glob.globlize(path_list)))
    assert "https://{a.com/file,b.com/b,c.com/file}.txt" == glob.globlize(
        path_list)
    assert sorted(path_list) == new_path_list
...
def test_ungloblize():
    ...
    assert ['/bucketForGlobTest/1.json'
