...
def test_write(client):
    writer = S3LimitedSeekableWriter(
        BUCKET, KEY, s3_client=client, block_size=4)
    ...
    assert writer._multipart_upload['Parts'] == [
        {
            'ETag': '"0f6430efebf97198427b56542a35c88d"',
            'PartNumber': 2
        }
    ]
    ...
    assert writer._multipart_upload['Parts'] == [
        ...
        {
            'ETag': '"816cabd1c0334ed363555889d9f4dbe4"',
            'PartNumber': 3
        },
    ]
...
def test_write_three_blocks(client):
    ...
    writer.seek(1)
    writer.write(b'A')
    assert writer._head_buffer.getvalue(
    ) == b'aA' + b'a' * (writer._head_block_size - 2)
    writer.seek(0, 2)
    writer.write(b'd')
    writer.close()

    body = client.get_object(Bucket=BUCKET, Key=KEY)['Body']
    assert body.read() == b'aA' + b'a' * (
        writer._head_block_size -
        1) + b'b' * writer._block_size + b'c' * writer._block_size + b'd'
...
def test_write_random(client):
    ...
    data = (
        ...
        random.randint(1, 100 * 1024 * 1024)).encode('ascii')
