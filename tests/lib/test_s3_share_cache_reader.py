...
def test_s3_share_cache_reader_readline(s3_empty_client):
    ...
    with S3ShareCacheReader(BUCKET, KEY, s3_client=s3_empty_client,
                            max_workers=2, block_size=3) as reader:
        # within block
...
def test_s3_share_cache_reader_close(client):
    ...
    with S3ShareCacheReader(BUCKET, KEY, s3_client=client, max_workers=1,
...
def test_s3_share_cache_reader_backward_seek_and_the_target_in_remains(
        client, mocker):
    '''目标 offset 在 remains 中 重置 remains 位置'''
    s3_share_cache_reader.DEFAULT_BLOCK_CAPACITY = 3
    with S3ShareCacheReader(BUCKET, KEY, s3_client=client, max_workers=2,
                            block_size=3, block_forward=2) as reader:

        reader.read(0)
        sleep_until_downloaded(reader)
        assert reader._cached_blocks == [
            ('s3://bucket/key', 1), ('s3://bucket/key', 0)
        ]
        ...
        sleep_until_downloaded(reader)
        assert reader._cached_blocks == [
            ('s3://bucket/key', 0), ('s3://bucket/key', 2),
            ('s3://bucket/key', 1)
        ]
        ...
        sleep_until_downloaded(reader)
        assert reader._cached_blocks == [
            ('s3://bucket/key', 2), ('s3://bucket/key', 1),
            ('s3://bucket/key', 0)
        ]
...
def test_s3_share_cache_reader_backward_seek_and_the_target_out_of_remains(
    ...
    with S3ShareCacheReader(BUCKET, KEY, s3_client=client, max_workers=2,
        ...
        sleep_until_downloaded(reader)
        assert reader._cached_blocks == [
            ('s3://bucket/key', 0), ('s3://bucket/key', 4),
            ('s3://bucket/key', 3)
        ]
        ...
        sleep_until_downloaded(reader)
        assert reader._cached_blocks == [
            ('s3://bucket/key', 3), ('s3://bucket/key', 1),
            ('s3://bucket/key', 0)
        ]
...
def test_s3_share_cache_reader_seek_and_the_target_in_buffer(client, mocker):
    ...
    '''
    s3_share_cache_reader.DEFAULT_BLOCK_CAPACITY = 3
    with S3ShareCacheReader(BUCKET, KEY, s3_client=client, max_workers=3,
                            block_size=3,
        ...
        sleep_until_downloaded(reader)
        assert reader._cached_blocks == [
            ('s3://bucket/key', 1), ('s3://bucket/key', 4),
            ('s3://bucket/key', 3)
        ]


def test_s3_share_cache_reader_seek_and_the_target_out_of_buffer(
        client, mocker):
    '''目标 offset 在 buffer 外，停止现有 future，丢弃当前 buffer，以目标 offset 作为新的起点启动新的 future'''
    s3_share_cache_reader.DEFAULT_BLOCK_CAPACITY = 3
    with S3ShareCacheReader(BUCKET, KEY, s3_client=client, max_workers=2,
