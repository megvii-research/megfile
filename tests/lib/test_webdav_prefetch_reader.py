...
def _fake_execute_request(*args, headers_ext=None, **kwargs):
    header_range = headers_ext and next(
        (h[7:] for h in headers_ext if h.startswith("Range:")), None
    )
    if header_range:
        start, end = list(map(int, header_range[6:].split("-")))
...
def test_webdav_prefetch_reader_readline(mocker):
    content = b"1\n2\n3\n\n4444\n5"
    mocker.patch(
        "megfile.lib.webdav_prefetch_reader.WebdavClient.execute_request",
...
def test_webdav_prefetch_reader_readline_tailing_block(mocker):
    mocker.patch(
        "megfile.lib.webdav_prefetch_reader.WebdavClient.execute_request",
...
def test_webdav_prefetch_reader_read_readline_mix(mocker):
    content = b"1\n2\n3\n4\n"
    mocker.patch(
        "megfile.lib.webdav_prefetch_reader.WebdavClient.execute_request",
        return_value=FakeResponse200(content),
...
def test_webdav_prefetch_reader_seek_out_of_range(mocker):
    content = b"1\n2\n3\n4\n"
    mocker.patch(
        "megfile.lib.webdav_prefetch_reader.WebdavClient.execute_request",
        return_value=FakeResponse200(content),
...
def test_webdav_prefetch_reader_backward_seek_and_the_target_in_remains(
    http_patch, mocker
):
...
def test_webdav_prefetch_reader_backward_block_forward_eq_1(http_patch, mocker):
    class FakeHistory:
        read_count = 1
...
def test_webdav_prefetch_reader_backward_seek_and_the_target_out_of_remains(http_patch):
    """
...
def test_webdav_prefetch_reader_seek_and_the_target_in_buffer(http_patch, mocker):
    """
...
def test_webdav_prefetch_reader_seek_and_the_target_out_of_buffer(http_patch):
    """
    目标 offset 在 buffer 外, 停止现有 future, 丢弃当前 buffer,
    以目标 offset 作为新的起点启动新的 future
    """
