...
def test_combine_reader_readline_without_line_break_at_all():
    ...
    with CombineReader(
        [io1, io2],
            'test_combine_reader_readline_without_line_break_at_all') as reader:
        reader.read(1)
...
def test_combine_reader_read_readline_mix():
    ...
    with CombineReader(io_list,
                       'test_combine_reader_read_readline_mix') as reader:
        assert reader.readline() == b'1\n'
...
def test_combine_reader_read_with_forward_seek():
    ...
    with CombineReader(
        [io1, io2, io3],
            'test_combine_reader_read_with_forward_seek_6') as reader:
        reader.seek(-1, os.SEEK_END)
...
def test_combine_reader_tell():
    ...
    with CombineReader([block0, block1, block2, block3, block4],
                       'test_combine_reader_tell') as reader:
        assert reader.tell() == 0
        reader.read(0)
        assert reader.tell() == 0
        reader.read(1)
...
def test_combine_reader_tell_after_seek():
    ...
    block4 = BytesIO(b'block4 ')
    with CombineReader([block0, block1, block2, block3, block4],
                       'test_combine_reader_tell_after_seek') as reader:
        reader.seek(2)
        assert reader.tell() == 2
        reader.seek(3)
