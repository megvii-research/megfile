...
def test_s3_cached_handler_generate_cache_path(client):
    writer = S3CachedHandler(
        BUCKET, KEY, 'wb', s3_client=client, cache_path=None)
...
def test_s3_cached_handler_read(client):
    ...
    with S3CachedHandler(BUCKET, KEY, 'rb', s3_client=client,
...
def test_s3_cached_handler_cache_file(client):
    ...
    with S3CachedHandler(BUCKET, KEY, 'rb', s3_client=client,
                         ...
                         remove_cache_when_open=False) as reader:
        assert reader.readline() == b'block0\n'
        assert reader.read() == b' block1\n block2'
...
def test_s3_cached_handler_write(client):
    with S3CachedHandler(BUCKET, KEY, 'wb', s3_client=client,
...
def test_s3_cached_handler_append(client):
    with S3CachedHandler(BUCKET, KEY, 'ab', s3_client=client,
                         cache_path=CACHE_PATH) as writer:
        writer.write(CONTENT)
...
def test_s3_cached_handler_mode_rb(client):
    ...
    with open(LOCAL_PATH, 'rb') as fp1, \
        S3CachedHandler(BUCKET, KEY, 'rb', s3_client=client,
                         cache_path=CACHE_PATH) as fp2:
...
def test_s3_cached_handler_mode_wb(client):
    ...
    with open(LOCAL_PATH, 'wb') as fp1, \
        S3CachedHandler(BUCKET, KEY, 'wb', s3_client=client,
                         cache_path=CACHE_PATH,
                         remove_cache_when_open=False) as fp2:
        assert_ability(fp1, fp2)
...
def test_s3_cached_handler_mode_ab(client):
    ...
    with open(LOCAL_PATH, 'ab') as fp1, \
        S3CachedHandler(BUCKET, KEY, 'ab', s3_client=client,
                         cache_path=CACHE_PATH,
                         remove_cache_when_open=False) as fp2:
        assert_ability(fp1, fp2)
        assert_write(fp1, fp2, CONTENT)
