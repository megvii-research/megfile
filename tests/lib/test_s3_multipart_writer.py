...
def test_s3_multipart_writer_close(client):
    writer = S3MultipartWriter(BUCKET, KEY, s3_client=client)
...
def test_s3_multipart_writer_write(client):
    with S3MultipartWriter(BUCKET, KEY, s3_client=client) as writer:
...
def test_s3_multipart_writer_write_max_worker(client, mocker):
    UNIT = 2**20
...
def test_s3_multipart_writer_write_put(client, mocker):
...
def test_s3_multipart_writer_write_large_bytes(client):
    with S3MultipartWriter(BUCKET, KEY, s3_client=client) as writer:
...
def test_s3_multipart_writer_write_multipart(client, mocker):
    block_size = 10 * 2**20
    content_size = 16 * 2**20
    content = b"a" * content_size
...
def test_s3_multipart_writer_write_multipart_pending(client, mocker):
    ...
    mocker.patch("megfile.lib.s3_multipart_writer.wait", side_effect=fake_wait)
...
def test_s3_multipart_writer_write_multipart_autoscale(client, mocker):
    block_size = 8 * 2**20
    content_size = 8 * 2**20
    content_repeat = 20
    content = b"a" * content_size
...
def test_s3_multipart_writer_autoscale_block_size(client, mocker):
    with S3MultipartWriter(
        BUCKET,
        KEY,
        s3_client=client,
